...
t_split		keep_split_support(char **s, t_split lst)
{
	...
	if ((*s)[lst.i] == '\\')
	{
		...
		if (esp % 2 == 0)
		{
			if (!(lst.tabs[lst.j] = malloc(lst.k + 1)))
			{
...
t_split		keep_split_support2(char *s, t_split lst, char c, char b)
{
	...
	lst.i -= lst.k;
	lst.k = 0;
	while (((s[lst.i] != c && s[lst.i] != b) || ((s[lst.i] == c || s[lst.i] == b) && lst.i - 1 >= 0 && s[lst.i-1] == '\\')) && s[lst.i] != '\0' )
	{
		lst.tabs[lst.j][(lst.k)++] = s[(lst.i)++];
	}
	lst.tabs[lst.j][lst.k] = '\0';
...
t_split		keep_split_support3(char *s, t_split lst, char c)
{
	(lst.i)++;
	(lst.k)++;
	while ((s[lst.i] != c || (lst.i-1 >=0 && s[lst.i-1] == '\\' && s[lst.i] == c)) && s[lst.i] != '\0')
	{
		(lst.i)++;
		(lst.k)++;
	}
	if (s[lst.i] == c && (lst.i == 0 || (lst.i-1 >= 0 && s[lst.i-1] != '\\')))
	{
		(lst.i)++;
		(lst.k)++;
	}
...
t_split		keep_split_core(char *s, t_split lst, char c, char b)
{
	...
	else if (s[lst.i] == c && (lst.i == 0 || (lst.i-1 >= 0 && s[lst.i-1] != '\\')))
	{
		lst = keep_split_support3(s, lst, c);
	}
	else if (s[lst.i] == b && (lst.i == 0 || (lst.i-1 >= 0 && s[lst.i-1] != '\\')))
	{
